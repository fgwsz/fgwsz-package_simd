...
class Packer {
public:
    static void run(const std::vector<std::filesystem::path>& inputs, const std::filesystem::path& output) {
        ...
        if (items.empty()) throw PackageError("No files to pack");
        ...
        if (!writer) throw PackageError("Failed to create output writer");
        ...
        for (size_t idx = 0; idx < items.size(); ++idx) {
            ...
            console::print("\r[" + std::to_string(idx+1) + "/" + std::to_string(items.size()) +
                           "] 正在打包: " + item.rel_path);
            std::flush(std::cout);
        ...
        writer.reset();
        console::print("\n");
        print_summary("打包完成", out_abs, items.size(), total_content_size, timer.elapsed());
        set_readonly(out_abs);
    }

private:
    struct Item { std::string rel_path; std::filesystem::path abs_path; };

    static void write_content(io::IWriter& writer, const std::filesystem::path& file, uint8_t key) {
        auto reader = io::create_reader(file, true);
        if (!reader) throw PackageError("Cannot open input file: " + path_utils::to_utf8(file));
    ...
    static std::vector<Item> collect_items(const std::vector<std::filesystem::path>& inputs, const std::filesystem::path& out_abs) {
        std::vector<Item> items;
        std::vector<std::filesystem::path> abs_inputs;
        for (const auto& p : inputs) {
            auto abs_p = std::filesystem::absolute(p);
            if (!std::filesystem::exists(abs_p)) throw PackageError("Path does not exist: " + path_utils::to_utf8(abs_p));
            if (std::find(abs_inputs.begin(), abs_inputs.end(), abs_p) != abs_inputs.end())
        ...
        for (const auto& p : abs_inputs) {
            ...
            if (std::filesystem::is_regular_file(p)) files.push_back(p);
            else if (std::filesystem::is_directory(p)) dirs.push_back(p);
            else throw PackageError("Unknown file type: " + path_utils::to_utf8(p));
        }

        std::map<std::string, std::vector<std::filesystem::path>> name_map;
        for (const auto& f : files) name_map[path_utils::to_utf8(f.filename())].push_back(f);
        for (const auto& [fname, vec] : name_map) {
            if (vec.size() > 1) {
                std::ostringstream msg;
                msg << "Duplicate direct filenames: " << fname << "\n";
                for (size_t i = 0; i < vec.size(); ++i)
                    msg << "  " << (i+1) << ". " << path_utils::to_utf8(vec[i]) << "\n";
        ...
        for (const auto& d : dirs) {
            ...
            for (auto& entry : std::filesystem::recursive_directory_iterator(d)) {
                if (std::filesystem::is_symlink(entry) || !std::filesystem::is_regular_file(entry)) continue;
                if (std::filesystem::absolute(entry.path()) == out_abs) continue;
    ...
    static bool confirm_overwrite(const std::filesystem::path& file) {
        console::println("警告: 输出文件 '" + path_utils::to_utf8(file) + "' 已存在，将被覆盖。");
    ...
    static void print_summary(const std::string& action, const std::filesystem::path& target,
        ...
        double ratio = pkg_size ? (static_cast<double>(content_size)/pkg_size*100) : 0;
        console::println(action + "，目标: " + path_utils::to_utf8(target));
        console::println("共处理 " + std::to_string(count) + " 个文件。");
        console::println("内容总大小: " + utils::format_size(content_size) + " (" + std::to_string(content_size) + " 字节)");
    ...
    static void set_readonly(const std::filesystem::path& p) {
        try {
            std::filesystem::permissions(p, std::filesystem::perms::owner_read | std::filesystem::perms::group_read | std::filesystem::perms::others_read,
