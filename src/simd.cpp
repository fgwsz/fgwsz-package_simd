...
#if defined(__AVX2__)
void xor_block_avx2(u8* dst, const u8* src, u8 key) {
...
void xor_block_sse2(u8* dst, const u8* src, u8 key) {
...
void xor_block_neon(u8* dst, const u8* src, u8 key) {
...
void xor_block(u8* dst, const u8* src, size_t len, u8 key) {
    ...
    for (; i + AVX2_SIZE <= len; i += AVX2_SIZE) {
        detail::xor_block_avx2(dst + i, src + i, key);
    }
#endif
    ...
    for (; i + SSE_SIZE <= len; i += SSE_SIZE) {
        detail::xor_block_sse2(dst + i, src + i, key);
    }
#elif defined(__aarch64__) || defined(_M_ARM64)
    constexpr size_t NEON_SIZE = 16;
    for (; i + NEON_SIZE <= len; i += NEON_SIZE) {
        detail::xor_block_neon(dst + i, src + i, key);
    }
#endif
